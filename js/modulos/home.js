...
function selectSubPestana(id){
    ...
    (pdfsGlobales[id]||[]).forEach((doc,index) => {
        const scale = (window.innerWidth <= 480) ? 0.75 : (window.innerWidth <= 768 ? 0.85 : 1);
        const widthPct = (100 / scale) + '%';
        const heightPct = (100 / scale) + '%';
        var item = `
            ...
            <div class="pdf-frame" style="overflow:hidden; position:relative; height:400px;">
                <iframe src="${doc.url}" frameborder="0" style="width:${widthPct}; height:${heightPct}; border:0; transform:scale(${scale}); transform-origin:0 0;"></iframe>
            </div>
